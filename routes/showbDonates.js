...
router.get("/", [auth], async (req, res) => {
	const bDonate = await BDonate.find({
		isCompleted: false,
		"bRequest.requestUser._id": "Nikita Karki",
	});
	res.send(bDonate);
});

router.get("/:id", [auth], async (req, res) => {
	const bDonate = await BDonate.find(req.params, {
		isCompleted: false,
		// "bRequest.requestUser._id": "62cbe0f6d978740dbecda25c",
		// "user._id": "62cbe127d978740dbecda265",
		// "bRequest.name": "Nikita Karki",
		// "bRequest.requestUser.name": "Nikita Karki",
		"bRequest.requestUser.phone": req.params.phone,
	});
	res.send(bDonate);
});
